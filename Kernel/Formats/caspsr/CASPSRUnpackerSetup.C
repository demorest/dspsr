...
void dsp::CASPSRUnpackerSetup::unpack()
{
  ...
  float* into_pola = output->getdatptr(0,0);
  float* into_polb = output->getdatptr(0,1);


  //uint64_t dataSize = ndat;
  int dimBlockUnpack(512);
  int dimGridUnpack(dataSize / dimBlockUnpack*8); 
  ...
  //call function
  caspsr_unpack(ndat,stagingBufGPU,unpackBufGPU,dimBlockUnpack,dimGridUnpack,halfData,into_pola,into_polb);
}
