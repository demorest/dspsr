...
{
  if (verbose)
    cerr << "Transformation::operate["+name+"]" << endl;

  if (!input)
    throw Error (InvalidState, "Transformation::operate["+name+"]",
		 "no input");
...
{
  if (verbose)
    cerr << "dsp::Transformation::set_input=" << _input << endl;

  input = _input;

  if ( type == outofplace && input && output
       && (const void*)input == (const void*)output )
    throw Error (InvalidState, "Transformation::set_input["+name+"]",
		 "input must != output");
}
...
{
  if (verbose)
    cerr << "dsp::Transformation::set_output=" << _output << endl;

  if (type == inplace)
    throw Error (InvalidState, "Transformation::set_output["+name+"]",
