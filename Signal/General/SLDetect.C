...
{
  if (verbose)
    cerr << "dsp::SLDetect::transformation ndim=" << input->get_ndim() << " npol=" << input->get_npol() 
	 << ((get_input()==get_output())?"inplace":"outofplace") << endl;
  ...
  {
    if (verbose)
      cerr << "dsp::SLDetect::transformation out of place" << endl;
    ...
    if (input->get_npol()==2)
      ...
      output->set_state( Signal::Intensity);
		     ...
		     "npol=%d not one or two", input->get_npol());
